...
class XMemTrainer:
    def __init__(self, config, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        self.XMem_freeze = nn.parallel.DistributedDataParallel(
            XMem(config).cuda(), 
            device_ids=[local_rank], output_device=local_rank, broadcast_buffers=False) 
    ...
    def do_pass(self, data, it=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.config['amp']):
            with torch.no_grad():
                # image features never change, compute once
                key, shrinkage, selection, f16, f8, f4 = self.XMem_freeze('encode_key', frames)

                filler_one = torch.zeros(1, dtype=torch.int64)
                hidden = torch.zeros((b, num_objects, self.config['hidden_dim'], *key.shape[-2:]))
                v16, hidden = self.XMem_freeze('encode_value', frames[:,0], f16[:,0], hidden, first_frame_gt[:,0])
                values = v16.unsqueeze(3) # add the time dimension
                ...
                for ti in range(1, self.num_frames):
                    ...
                    # Segment frame ti
                    memory_readout = self.XMem_freeze('read_memory', key[:,:,ti], selection[:,:,ti] if selection is not None else None, 
                                            ref_keys, ref_shrinkage, ref_values)
                    hidden, logits, masks = self.XMem_freeze('segment', (f16[:,ti], f8[:,ti], f4[:,ti]), memory_readout, 
                            hidden, selector, h_out=(ti < (self.num_frames-1)))

                    # No need to encode the last frame
                    if ti < (self.num_frames-1):
                        is_deep_update = np.random.rand() < self.deep_update_prob
                        v16, hidden = self.XMem_freeze('encode_value', frames[:,ti], f16[:,ti], hidden, masks, is_deep_update=is_deep_update)
                        values = torch.cat([values, v16.unsqueeze(3)], 3)

                    out[f'pre_masks_{ti}'] = masks
                    out[f'pre_logits_{ti}'] = logits
    ...
    def load_network_in_memory(self, src_dict):
        ...
        self.XMem_freeze.module.load_weights(src_dict) 
        self.freeze() 
    ...
    def freeze(self):
        for p in self.XMem_freeze.parameters():
            p.requires_grad = False
        
        self.XMem_freeze.eval()