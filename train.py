...
from dataset.vos_dataset_tao import VOSDatasetTAO

from util.logger import TensorboardLogger
from util.configuration import Configuration
from util.load_subset import load_sub_davis, load_sub_yv, load_sub_tao
...
for si, stage in enumerate(stages_to_perform):
    ...
    def renew_vos_loader(max_skip, finetune=False):
        ...
        yv_dataset = VOSDataset(path.join(yv_root, 'JPEGImages'), 
                            ...
                            max_skip//5, is_bl=False, subset=load_sub_yv(),
        ...
        davis_dataset = VOSDataset(path.join(davis_root, 'JPEGImages', '480p'), 
                            ...
                            max_skip, is_bl=False, subset=load_sub_davis(),
                            num_frames=config['num_frames'], finetune=finetune)
        
        tao_dataset = VOSDatasetTAO(path.join(tao_root, 'JPEGImages'), 
                                path.join(tao_root, 'Annotations'),
                                max_skip//5, is_bl=False, subset=load_sub_tao(),
                                num_frames=config['num_frames'], finetune=finetune)
        
        if config['data_pick'] == 'all':
            train_dataset = ConcatDataset([davis_dataset]*5 + [yv_dataset])
            
        # DAVIS2017
        elif config['data_pick'] == 'd17':
            train_dataset = ConcatDataset([davis_dataset] * 5) 

        # YouTube
        elif config['data_pick'] == 'yv':
            train_dataset = ConcatDataset([yv_dataset])
        
        # tao2017
        elif config['data_pick'] == 'tao':
            train_dataset = ConcatDataset([tao_dataset] * 5) 
    ...
    if stage == '0':
        ...
        tao_root = path.join(path.expanduser(config['tao_root']), 'train_480p')
