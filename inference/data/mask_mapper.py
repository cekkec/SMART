...
class MaskMapper_TAO:
    ...
    def convert_mask(self, mask, exhaustive=False):
        ...
        # Compute new labels to add to the remappings
        new_labels = list(set(labels) - set(self.labels))
        
        # Add new remappings for any new labels found
        for l in new_labels:
            self.remappings[l] = len(self.labels) + 1
            self.labels.append(l)
            if self.coherent and self.remappings[l] != l:
                self.coherent = False

        # Remap all labels in the mask to their new continuous indices
        remapped_mask = np.zeros_like(mask)
        for old_label in labels:
            new_label = self.remappings.get(old_label, old_label)
            remapped_mask[mask == old_label] = new_label

        # Convert the remapped mask to a one-hot representation
        mask_one_hot = torch.from_numpy(all_to_onehot(remapped_mask, self.labels)).float()

        return mask_one_hot, self.labels
