...
class VOSDatasetTAO(Dataset):
    ...
    def __init__(self, im_root, gt_root, max_jump, is_bl, subset=None, num_frames=3, max_num_obj=3, finetune=False):
        ...
        # Get video section directories first
        section_list = sorted(os.listdir(self.im_root))
        
        for section in section_list:
            section_path = os.path.join(self.im_root, section)
            if not os.path.isdir(section_path):
                continue

            # Iterate through each video in the section
            vid_list = sorted(os.listdir(section_path))

            # Pre-filtering
            for vid in vid_list:
                vid_path = os.path.join(section_path, vid)
                vid = f"{section}/{vid}"
                ...
                # Filter out files starting with '._' - MOSE
                frames = [f for f in frames if not f.startswith('._')] 
                
                if len(frames) < num_frames:
                    continue
                
                # Store the frames with full paths for access
                self.frames[vid] = [f for f in frames]
    ...
    def __getitem__(self, idx):
        ...
        while trials < 5:
            ...
            for f_idx in frames_idx:
                ...
                reseed(sequence_seed)
                this_im_crop = Image.open(path.join(vid_im_path, jpg_name)).convert('RGB')
                this_im_crop = self.all_im_dual_transform(this_im_crop)
                to_tensor = transforms.ToTensor()
                this_im_crop = to_tensor(this_im_crop)
                ...
                org_images.append(this_im_crop)  
                masks.append(this_gt)
                
            images = torch.stack(images, 0)
            org_images = torch.stack(org_images, 0)
